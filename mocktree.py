...
class MockParseTree:
    ...
    def __init__(self, json_node: Dict[str, Any], parser_rule_names: List[str] = None):
        ...
        # Extract rule/token name and set up class name
...
# Create a separate class that your visitor can check
class MockContextClass:
    """Mock class for child.__class__.__name__ access"""
    def __init__(self, name):
        self.__name__ = name


# Monkey patch the MockParseTree to have the right class behavior
def get_mock_class(self):
    """Return mock class for visitor compatibility"""
    return MockContextClass(self.class_name)

# Add the __class__ property after class definition to avoid recursion
MockParseTree.__class__ = property(get_mock_class)
