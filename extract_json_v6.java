...
public class runtime_measure {
    ...
    // Inner class to represent file processing result
    public static class FileProcessingResult {
        ...
        public FileProcessingResult(String filename, long durationInSec, String jsonFilename, String status) {
            this.filename = filename;
        ...
        public String toCsvLine() {
            return filename + "," + durationInSec + "," + jsonFilename + "," + status;
        }
    }
    ...
    // Thread-safe logging lists
    private static final List<String> timeoutFiles = Collections.synchronizedList(new ArrayList<>());
    private static final List<String> timeoutQueries = Collections.synchronizedList(new ArrayList<>());
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("Using " + numProcessors + " processor threads for parallel processing");

        // Create thread pool for parallel processing
        ExecutorService mainExecutor = Executors.newFixedThreadPool(numProcessors);
        ...
        for (Path file : filesToProcess) {
            Future<FileProcessingResult> future = mainExecutor.submit(() -> {
                return processFile(file, jsonOutputFolder, gson);
            });
            futures.add(future);
        }
        ...
        lines.addAll(results.stream().map(FileProcessingResult::toCsvLine).collect(Collectors.toList()));
    ...
    /**
     * Process a single file (called by parallel threads)
     */
    private static FileProcessingResult processFile(Path file, Path jsonOutputFolder, Gson gson) {
        String filename = file.getFileName().toString();
        System.out.println("Processing: " + filename + " [Thread: " + Thread.currentThread().getName() + "]");
        ...
        try {
            ...
            if (result != null) {
                ...
                // Save JSON to file - serialize only the tree structure
                String jsonFilename = filename.replace(".txt", ".json");
                Path jsonFilePath = jsonOutputFolder.resolve(jsonFilename);
                String jsonString = gson.toJson(result.parseTree);
                Files.writeString(jsonFilePath, jsonString);

                System.out.println("JSON saved to: " + jsonFilePath + " [Thread: " + Thread.currentThread().getName() + "]");
                ...
                return new FileProcessingResult(filename, durationInSec, jsonFilename, result.metadata.status);
            } else {
                ...
                // Step 2: Extract successful queries with individual timeouts
                String concatenatedQueries = extractAndConcatenateSuccessfulQueries(input, filename);
                
                endTime = System.nanoTime();
                durationInSec = (endTime - fileStartTime) / 1_000_000_000;
                
                if (concatenatedQueries != null && !concatenatedQueries.isEmpty()) {
                    System.out.println("Concatenated successful queries. Re-parsing with infinite timeout...");
                    
                    // Step 3: Re-parse concatenated queries with infinite timeout
                    ParseResult finalResult = processFullFileWithInfiniteTimeout(concatenatedQueries, filename);
                    
                    if (finalResult != null) {
                        finalResult.metadata.processingTime = durationInSec + "s";
                        
                        // Save JSON to file with original filename
                        String jsonFilename = filename.replace(".txt", ".json");
                        Path jsonFilePath = jsonOutputFolder.resolve(jsonFilename);
                        String jsonString = gson.toJson(finalResult.parseTree);
                        Files.writeString(jsonFilePath, jsonString);

                        System.out.println("Final JSON saved to: " + jsonFilePath);
                        System.out.println("Total tokens: " + finalResult.tokenStream.size());
                        System.out.println(filename + " completed with query splitting in " + durationInSec + "s - Status: PARTIAL");

                        return new FileProcessingResult(filename, durationInSec, jsonFilename, "PARTIAL");
                    } else {
                        System.out.println(filename + " failed final re-parsing");
                        return new FileProcessingResult(filename, durationInSec, "FAILED", "REPARSE_FAILURE");
                    }
                } else {
                    System.out.println(filename + " had no successful queries");
                    return new FileProcessingResult(filename, durationInSec, "FAILED", "NO_SUCCESSFUL_QUERIES");
                }
            }
        } catch (Exception e) {
            ...
            long endTime = System.nanoTime();
            long durationInSec = (endTime - fileStartTime) / 1_000_000_000;
            return new FileProcessingResult(filename, durationInSec, "FAILED", "EXCEPTION");
        }
    }
    ...
    /**
     * Extract successful queries and concatenate them
     */
    private static String extractAndConcatenateSuccessfulQueries(String input, String filename) {
        String[] queries = input.split(";");
        List<String> successfulQueries = new ArrayList<>();
        
        int totalQueries = queries.length;
        System.out.println("Testing " + totalQueries + " individual queries from " + filename);
        ...
        for (int i = 0; i < queries.length; i++) {
            ...
            System.out.println("Testing query " + (i + 1) + "/" + totalQueries);
            
            if (isQuerySuccessful(query, queryIdentifier)) {
                successfulQueries.add(query);
                System.out.println("Query " + (i + 1) + " successful");
            } else {
                // Log failed query
        ...
        System.out.println("Successfully extracted " + successfulQueries.size() + "/" + totalQueries + " queries");
        
        // Concatenate successful queries with semicolons
        return successfulQueries.isEmpty() ? null : String.join("; ", successfulQueries);
    }

    /**
     * Test if a single query can be parsed successfully within timeout
     */
    private static boolean isQuerySuccessful(String query, String queryIdentifier) {
        ParseResult result = processQueryWithTimeout(query, queryIdentifier);
        return result != null && result.metadata.status.equals("SUCCESS");
    }

    /**
     * Process with infinite timeout (no ExecutorService)
     */
    private static ParseResult processFullFileWithInfiniteTimeout(String input, String identifier) {
        try {
            System.out.println("Processing concatenated queries with infinite timeout...");
            return processFullFile(input, identifier);
        } catch (Exception e) {
            System.err.println("Error processing concatenated queries for " + identifier + ": " + e.getMessage());
            return null;
        }
    }
