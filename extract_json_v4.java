...
public class runtime_measure {
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(folderPath, "*.txt")) {
            ...
            for (Path file : stream) {
                ...
                if (result != null) {
                    ...
                    List<TreeNode> combinedTrees = processFileByQueriesReturnTrees(input, filename);
                    
                    endTime = System.nanoTime();
                    durationInSec = (endTime - fileStartTime) / 1_000_000_000;
                    
                    if (combinedTrees != null && !combinedTrees.isEmpty()) {
                        // Save combined JSON to file with original filename
                        String jsonFilename = filename.replace(".txt", ".json");
                        Path jsonFilePath = jsonOutputFolder.resolve(jsonFilename);
                        String jsonString = gson.toJson(combinedTrees);
                        Files.writeString(jsonFilePath, jsonString);

                        System.out.println("Combined JSON saved to: " + jsonFilePath);
                        System.out.println(filename + " partial processing took " + durationInSec + "s - Status: PARTIAL");
    ...
    /**
     * Process file by splitting into individual queries - returns list of trees
     */
    private static List<TreeNode> processFileByQueriesReturnTrees(String input, String filename) {
        ...
        for (int i = 0; i < queries.length; i++) {
            ...
            if (queryResult != null && queryResult.metadata.status.equals("SUCCESS")) {
                // Add tree from successful query
        ...
        return successfulTrees.isEmpty() ? null : successfulTrees;
    }
