...
public class SQLQueryValidator {
    ...
    public static FileStats processSqlFile(String filePath, String startRule, String outputDir, String csvFilePath) {
        ...
        try {
            ...
            // Append to CSV file
            synchronized (SQLQueryValidator.class) {
    ...
    public static void processAllFiles(String inputDir, String outputDir, String csvFilePath) {
        ...
        try {
            ...
            try (PrintWriter writer = new PrintWriter(new FileWriter(csvFilePath))) {
                writer.println("file_name,number_of_queries,queries_with_error,corrupted_queries,percentage_of_errors,time_taken");
            }
            ...
            for (int i = 0; i < futures.size(); i++) {
                try {
                    futures.get(i).get(TIMEOUT_MINUTES, TimeUnit.MINUTES);
                } catch (TimeoutException e) {
                    countOfTimeTakingCodes++;
                    System.err.println("Skipping file as it took too long: " + txtFiles.get(i));
                    System.out.println(countOfTimeTakingCodes + " -*100");
                    futures.get(i).cancel(true);
                } catch (ExecutionException | InterruptedException e) {
                    System.err.println("Error processing file " + txtFiles.get(i) + ": " + e.getMessage());
                }
            }
